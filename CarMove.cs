...
public class CarMove : MonoBehaviour
{
    ...
    {
        new Vector3(-81.8f, -7.1f, -11.2f),  // 第一段：X轴移动（无需转向）
        new Vector3(-81.8f, -7.1f, 17.1f),   // 第二段：Z轴移动（左转-90°）
        new Vector3(-54.5f, -7.1f, 17.1f)    // 第三段：X轴移动（右转+90°）
    };
    ...
    void Start()
    {
        ...
        transform.position = new Vector3(-216.67f, -7.1f, -11.2f);
/*        transform.position = new Vector3(-81.8f, -7.1f, -11.2f);*/
    ...
    IEnumerator MoveAlongPath()
    {
        while (currentWaypoint < waypoints.Length)
        {
            ...
            // 阶段转向控制
            if (currentWaypoint == 1) // 到达第一个点后左转-90°
            {
                yield return StartCoroutine(RotateTruck(-90f));
            }
            else if (currentWaypoint == 2) // 到达第二个点后右转+90°
            {
                yield return StartCoroutine(RotateTruck(90f));
            }
